...
const createShow = async (eventId: string, showData: IShow): Promise<IShow> => {
  ...
  const isExist = await Show.findOne({ event: eventId, startTime: showData.startTime, endTime: showData.endTime });
  if (isExist) {
    throw new ApiError(httpStatus.CONFLICT, 'Buổi biểu diễn đã tồn tại.');
  }

  showData.event = eventId as unknown as ObjectId;
...
const getShowsOfEvent = async (eventId: string): Promise<IShow[]> => {
  ...
  const shows = await Show.find({ event: eventId }).populate('ticketTypes').lean();
  if (!shows) {
...
const getShow = async (eventId: string, showId: string): Promise<IShow> => {
  ...
  const show = await Show.findOne({ event: eventId, _id: showId }).populate('ticketTypes').lean();
  if (!show) {
...
const deleteShow = async (eventId: string, showId: string): Promise<void> => {
  ...
  await Show.deleteOne({ event: eventId, _id: showId });
};
